...
m <- Mapper:::MapperRef$new(as.matrix(WV_US_nice))
m$use_cover(filter_values=wv_ica$S, type="fixed rectangular", number_intervals=15L, percent_overlap=15)$
  use_distance_measure(measure = "euclidean")$
  use_clustering_algorithm(cl = "single", num_bins = 10L)$
  compute_vertices()$
  compute_edges(min_weight = 15L)


Mapper::dashboard(m, WV_US_nice)


# m$clustering_algorithm <- function(X, idx){ rep(1L, length(idx)) }

plot(wv_ica$S, col = bin_color(WV_US_nice$FamilyImportance))
...
```{r}
d <- 3L
test_cover <- Mapper::FixedRectangularCover$new(filter_values = replicate(d, rnorm(10)))
test_cover$percent_overlap <- 25
sapply(2:5, function(k) {
  test_cover$number_intervals <- k 
  test_cover$construct_cover() 
  nrow(test_cover$level_sets_to_compare())
}) #13 x3 + 12 x2 + 3 x 
```
