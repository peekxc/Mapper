...
make_line_plot <- function(x, title, sub_title){
  ...
  y_breaks <- ceiling(with(x, { seq(0, max(counts), length.out = 10) }))
  ...
  xlab("Percent Overlap") + ylab(label = "Cum. # Intersection Comparisons\n") + 
...
local({
  ...
  wh_ratio <- (page_height*0.23)/page_width ## To get figure aspect ratio to whatever percentage height of page
  res <- 4 ## to increase/decrease resolution
  { pt_width <- page_width*res; pt_height <- wh_ratio*pt_width } 
  pdf(file = "set_results.pdf", width = pt_width/72, height = pt_height/72, compress = TRUE)
