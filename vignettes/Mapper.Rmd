...
One way to fix this is to adjust the parent environment of the closures execution environment with the environment of the `MapperRef` instance. Since all public functions are [enclosured in their instance environment](https://github.com/r-lib/R6/blob/master/doc_extra/R6.pdf), the `initialize` members environment will do.
