...
c_env[["Mean filter value"]] <- function(M, ...){
  agg_pt_fv <- sapply(M$G$nodes, function(n_idx){ apply(as.matrix(M$cover$filter_values[n_idx,]), 1, mean)})
  agg_node_val <- sapply(agg_pt_fv, mean)
  return(agg_node_val)
}

## Computes the mean data value for each node
c_env[["Mean data value"]] <- function(M, X, ...){
  agg_pt_x <- sapply(M$G$nodes, function(n_idx){ apply(as.matrix(X[n_idx,]), 1, mean) })
...
c_env[["Density"]] <- function(M, ...){
  return(sapply(M$G$nodes, length))
}
