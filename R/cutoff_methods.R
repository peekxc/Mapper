...
cutoff_first_bin <- function(hcl, num_bins) {
  ...
  breaks <- as.double(seq(min(hcl$height), max(hcl$height), length.out = num_bins))
  ...
  ## If there's a uniform distribution of distances, every point is assigned to the same cluster
