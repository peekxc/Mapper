...
#' @details This method constructs a sequence of Mappers connected by simplicial maps. 
#' Because this is a computationally expensive operation, a special cover indexing structure is used 
#' to express the computation in terms of elementary simplicial maps. This method requires a fixed interval 
#' cover with a constant number of open sets, and uses a specific clustering algorithm. \cr
#' \cr
...
#' however they will produce visually different persistence diagrams. 
#' @export 
multiscale <- function(m, max_dim=1L, max_overlap = 50, time = c("integer", "measure", "overlap"), f = NULL, stats = FALSE){
  ...
  stopifnot(m$cover$percent_overlap == 0)
  
  ## Get initial eps parameter to minimize bias on the clusters
  global_eps <- mean(sapply(m$cover$index_set, function(alpha){
    hcl <- hclust(dist(m$X(m$cover$construct_cover(alpha))), method = "single")
  ...
  distinct <- local({
    base_eps <- diff(apply(m$cover$filter_values, 2, range))/m$cover$number_intervals
    distinct_g <- matrix(apply(distinct$eps, 1, function(eps) 1 - (base_eps / eps)), nrow = ncol(m$cover$filter_values)) 
  ...
  for (i in seq(nrow(distinct$idx))){
    # browser()
    tidx <- distinct$idx[i,]
    changes <- indexed_cover$update_segments(tidx)
    
    ## To save adjacent pullback indices 
    pairs_to_update <- vector(mode = "list", length(changes$indices_changed))
    cc <- 1L
    ...
    for (a_i in changes$indices_changed){
      ...
      ## This is equivalent to getting the cofaces of the vertices in the current pullback to update.
      adj_pids <- Mapper:::connected_pullbacks(pid_to_update, m$pullback, m$simplicial_complex$as_XPtr())
      if (length(adj_pids) > 0){
        pairs_to_update[[cc]] <- as.matrix(expand.grid(pid_to_update, adj_pids[[pid_to_update]]))
        cc <- cc + 1L
      }
    ...
    idx_pairs <- matrix(apply(changes$pairs_changed, 2, function(idx){ m$cover$index_set[idx+1L] }), ncol = 2)
    idx_pairs <- unique(rbind(idx_pairs, do.call(rbind, pairs_to_update)))
    
    ## Update the nerve locally
    simplices <- m$construct_nerve(indices = idx_pairs, k = max_dim, modify = FALSE)
  ...
  if (missing(time) || time == "integer"){
    ...
    base_eps <- diff(apply(m$cover$filter_values, 2, range))/m$cover$number_intervals
    time <- as.numeric(colMeans(matrix(apply(distinct$eps, 1, function(eps) 1 - (base_eps / eps)), nrow = ncol(m$cover$filter_values))))
  } else {
  ...
  result <- list(initial_mapper = initial, simplicial_maps = s_maps)
  return(result)
}
...
construct_indexed_cover <- function(cover, ensure_unique = FALSE){
  stopifnot(cover$typename == "Fixed Interval")
  stopifnot(cover$percent_overlap == 0)
  
  ## Locals
  n <- nrow(cover$filter_values)
  d <- ncol(cover$filter_values)
  base_interval_length <- diff(apply(cover$filter_values, 2, range))/cover$number_intervals
  ...
  pt_flat_idx <- Mapper:::constructLevelSetIndex(cover$filter_values, interval_bnds)
  ...
  ## Calculate point-to-set distances to non-intersecting open sets
  {
    ...
    dist_to_lb <- matrix(sapply(seq(n), function(i){ abs(cover$filter_values[i,] - lb[i,]) }), nrow = d)
    dist_to_ub <- matrix(sapply(seq(n), function(i){ abs(cover$filter_values[i,] - ub[i,]) }), nrow = d)
