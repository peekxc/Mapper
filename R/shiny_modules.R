...
MapperVis <- modules::module({
  ...
  ShinyCover <- function(input, output, session){
    cover <- CoverRef$new(typename = "fixed interval")
    cover$number_intervals <- input$number_intervals
    cover$percent_overlap <- input$percent_overlap
    return( reactive({ cover }) )
  }
  ...
  ShinyMultiscaleMapper <- function(input, output, session, initial_complex, smaps, pp_shiny){
    ...
    observeEvent(input$eps, {
      req(pp_shiny$pp_vis())
      # browser()
      ub_idx <- ifelse(input$eps < min(bd), 0, max(which(bd <= input$eps)))
