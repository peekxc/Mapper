...
#'
#' Cover reference class - part of Reference Class (R5) implementation of Mapper for the Mapper package in R
...
cover_ref <- setRefClass("Cover", fields = c("filter_values", "index_set", "level_sets", "type"))

## Cover initialization
cover_ref$methods(initialize = function(filter_values, type = "rectangular"){
...
cover_ref$methods(getLSMI = function(lsfi){
  index_set[lsfi,]
})

cover_ref$methods(getLSFI = function(lsmi){
...
# cover_ref$methods(computeLevelSets = function(){
...
cover_ref$methods(computeOverlap = function(box_sizes){
...
cover_ref$methods(computeHalfSpaceDistances = function(){
...
cover_ref$methods(computeTargetLSFI = function(){
  if ("uninitializedField" %in% class(num_intervals)){ stop("'num_intervals' must be set prior to computing all overlap values.") }
...
cover_ref$methods(plotFilterSpace = function(show_ls_bounds = TRUE, show_lsfi = FALSE, ...){
