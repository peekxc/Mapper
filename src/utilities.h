...
// Rather than passing the elements of the sets to enumerate directly, 
// this version supplies f with the (0-based) indices forming the product
template <typename T, typename Func> 
inline void cart_prod(const vector< T >& sizes, Func&& f) {
  static_assert(std::is_integral<T>::value, "Integral required.");
  // bool any_0 = std::any_of(begin(sizes), end(sizes), [](T sz){ return(sz == 0); }); 
  // if (any_0){ stop("Invalid set of sizes."); }
    
  // Initialize the slots to hold the current iteration value for each depth
  const size_t depth = sizes.size();
  size_t* slots = (size_t*) alloca(sizeof(size_t) * depth);
  std::fill_n(slots, depth, 0);
  vector< size_t > idx(depth);

  // Cascade 
  size_t index = 0;
  while (true) {
    idx.assign(slots, slots+depth);
    f(idx); // Apply function
    slots[0]++; // Increment + Carry
    while (slots[index] >= sizes.at(index)) {
      if (index == depth - 1) { return; } // Overflow, we're done
      slots[index++] = 0;
      slots[index]++;
    }
...
// to the lambda function f. If f returns false, the loop is ended. 
