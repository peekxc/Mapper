...
vector< IntegerVector > apply_clustering(const NumericMatrix& X, const IntegerVector& level_set, const Function f){
...
// TODO: cannot used this for simplicial maps, as will remove all vertices in the pullback. 
void update_level_set(const int ls_flat_index, const IntegerVector level_set, const NumericMatrix& X, const Function f, std::map< int, IntegerVector >& v_map, List& ls_vertex_map, SEXP stree){
  ...
  vector< std::size_t > v = to_vec(c_vertices);
  
  vector< std::size_t > vertex = { 0 };
  std::for_each(c_vertices.begin(), c_vertices.end(), [&vertex, &stree_ptr](const int v){
    vertex[0] = v;
    stree_ptr->remove_simplex(vertex);
  });
  ...
  if (level_set.size() > 0){
    ...
    vector< IntegerVector > new_vertices = apply_clustering(X, level_set, f);
    const std::size_t n_new_v = new_vertices.size(); 
      
    // Update the map with new vertex map with the new indices
    vector< idx_t > new_0_simplexes = stree_ptr->generate_ids(n_new_v);
    ...
    std::for_each(new_0_simplexes.begin(), new_0_simplexes.end(), [&stree_ptr](const int v_i){
      vector< size_t > v = { static_cast< size_t >(v_i) };
...
IntegerVector check_connected(const IntegerVector ls_to_check, const List& ls_vertex_map, const List& vertices, SEXP stree){
  ...
  std::for_each(ls_to_check.begin(), ls_to_check.end(), [&](const int c_ls){
    ...
    std::for_each(c_vertices.begin(), c_vertices.end(), [&](const int v_i){
      const IntegerVector adj_vertices = to_ivec(stree_ptr->adjacent_vertices(v_i));
