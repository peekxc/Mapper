...
[![Package status](https://www.tidyverse.org/lifecycle/#maturing)]
...
- Efficient implementation of *Mapper* components using [Rcpp](https://cran.r-project.org/web/packages/Rcpp/index.html)
...
Constructing the simplicial complex requires applying the _pullback_ operation prior to constructing nerve. The pullback applies the clustering algorithm to subsets of the data given by the cover, which decomposes the data set into connected components. In Mapper, these connected components are represented as vertices. To view the mapping from open sets to vertices, use the `pullback` member:
```{r}
...
The complex is stored in a [Simplex Tree](?simplex_tree) (see 2), and the vertices are stored are stored as a named list, where each name corresponds to the 0-simplex in the simplicial complex, and each value denotes a vector of the points indices that comprise the connected component. 
```{r}
sapply(m$vertices, length)
```

Once you're satisfied with the clustering, you can construct the nerve by considering the $k+1$-fold non-empty intersections between the sets in the pullback. It's common to restrict the degree of the intersections considered, e.g. to ensure the *mapper* is a graph, use $k=1$.
```{r}
...
Alternatively, the $1$-skeleton can be exported to any of the usual graph-type data structures.
```{r, eval=FALSE}
...
These graph data-structures can then be converted to [igraph](https://igraph.org/r/) objects and customized however needed. A default export option is available:
